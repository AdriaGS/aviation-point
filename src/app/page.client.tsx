...
export default function HomeClient({ blogPosts }: { blogPosts: BlogPost[] }) {
  const { location } = useLocationHandler();
  ...
  useEffect(() => {
    if (location) {
      ...
      getNearestAirports(location.coords.latitude, location.coords.longitude)
        .then(airports => {
          setAirports(airports);
          // Get weather for each airport
          Promise.all(airports.map((airport: AirportData) =>
            getWeatherAtLocation(airport.latitude, airport.longitude)
          )).then(setWeather);
          // Get flights from closest airport
          if (airports[0]?.iataCode) {
            getDepartingFlights(airports[0].iataCode)
